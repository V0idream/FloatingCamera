...
**Floating Camera** 是一个原生 Android 悬浮窗相机示例项目，用于演示如何在合规权限流程下结合悬浮窗、CameraX、前台服务和本地文件管理能力。

项目适合用于学习 Android 悬浮窗交互、运行时权限、相机/录像调用、前台服务通知以及应用专用目录文件管理。它不是用于绕过系统相机、麦克风或隐私提示的工具。

## 免责声明

本软件仅供技术交流与合法学习使用，禁止用于非法用途。使用者应自行遵守所在地区的法律法规，以及设备、平台和应用市场规则。由此产生的一切后果由使用者自负，项目作者不承担任何责任。
...
This software is provided only for technical discussion and lawful learning. Illegal use is prohibited. Users are responsible for complying with local laws, device rules, platform policies, and app store requirements. The project author is not responsible for any consequences caused by misuse.
